...
#-------------------------------------------------------------------------------------
    ...
    '''
    Initialize time differences vectors
    '''
    def dtvector(self):
    ...
    '''
    Initialize the global vectors: acceleration, velocity, position, angle, and displacement
    '''
    ...
    '''
    Initialize the local orientations matrix defined in the main file
    '''
	...
	#-------------------------------------------------------------------------------------
    '''
    Calculate global accelerations, angles and coordinate system by applying the SORA method over the range of
    ...
    def cycleovertimestep(self):	   
        ...
        for i in range (0,self.nts-1):
            ...
            '''
            Calculate the directional cosines to define the position of the sensor with respect to global coordinate
            system
            '''
		# Calculate directional cosines
            ...
            '''
            Generate rotational matrix to define the position of the sensor with respect to global coordinate
            system
            '''
		 # Generate rotational matrix
            ...
            '''
            Obtain the global acceleration using the rotational matrix
            '''
		# Rotate local acceleration
            ...
            '''
            Obtain the global angle using the rotational matrix
            '''
		# Rotate local velocity
            self.g_angl[i,0:3]= np.dot(self.RM.RotationMatrixAA(),np.matrix.transpose(self.l_angles[i,0:3]))
            
            '''
            Calculate the rotational angle and the rotational vectors for SORA method
            '''
		# Calculate angle for simultaneous orthogonal rotations
            ...
            '''
            Generate the rotational matrix for SORA method
            '''
		# Calculate rotational matrix
            ...
            '''
            Rotate the local coordinate system into new local coordinate system
            '''
    ...
    def globalvalue(self):
        ...
        ''' 
        Calculate the global values: velocity, position, and displacement using already known global acceleration
        '''
	...
	#-------------------------------------------------------------------------------------
    '''
    Define the getter function so that we can use the needed variables into another class
    '''
    ...
    #------------------------------------------------------------------------------------- 
    '''
    Print the final result into csv file to get a clear understanding of the result
    '''
